...
for (task in c(1, 2, 3)){
  for (data_type in c('raw', 'normalized', 'batchCorrected')){
    ...
    train_x <- train_x[rowSums(is.na(train_x)) / ncol(train_x) <= 0, ]
    ...
    # challenge_x1 <- challenge_x[, !names(challenge_x) %in% outlist]
    # pred <- quickpred(challenge_x1, mincor = 0.0, method = 'pearson')
    # 
    # challenge_x_imp <- mice(challenge_x1, m = 5, maxit = 10, pred = pred, seed = 100)
